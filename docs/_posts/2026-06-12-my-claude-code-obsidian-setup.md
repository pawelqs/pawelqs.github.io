...
Using Obsidian bases, I can now turn jobs on and off and update their schedules from a single view in Obsidian. What do I use it for? For e.g. to monitor the aurora forecast and to send me the notification if the forecast's promising, or to check and summarize the RSS feed from websites, or to update weather forecast in planning note of my next holidays - usually using the skills and CLIs created just a moment ago. I use it to automate simple tasks that I rarely remember about, and to provide the summary information exactly to the place (note), where I will need it.
